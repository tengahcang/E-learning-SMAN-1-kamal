...
@section('content')
    ...
    <div class="container bg-white p-3">
        ...
        <form action="{{ route('teacher.matapelajaran.update',$activity->id) }}" method="POST" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="form-group mb-4">
                <label for="name">Nama Pertemuan</label>
                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                    value="{{ $errors->any() ? old('name') :$activity->name }}" required autocomplete="name" autofocus id="namaPertemuan"
                ...
                <input id="room" type="hidden" class="form-control @error('name') is-invalid @enderror" name="room"
                    value="{{ $activity->id_room }}" required autocomplete="room" autofocus>
                @error('name')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <button type="submit" class="btn text-black btn-tambahkan">Update</button>
        </form>
    </div>
